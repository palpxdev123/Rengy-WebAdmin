    ...
    <div className="rounded-[4px]  bg-white mt-[24px]">
      <div className="p-[20px] h-[65px] border-b-[1px] border-header items-center">
        <div className="flex justify-between">
          ...
          <p className=" pr-[8px] view_all">View All</p>
        </div>
      </div>
      <div className="m-[19px] p-[4px] rounded-[40px] main-bg flex justify-between items-center scrollbarHidden overflow-x-scroll">
        {
          tabs?.map((item)=>(
            <div onClick={()=>setActive(item)} className={`${active === item && "bg-[white]"} py-[6px] px-[12px] min-w-[170px] flex justify-center items-center rounded-[42px]`}>
              {item}
            </div>
          ))
        }
          ...
          Stagewiseproject?.map((item)=>(
            <div className="px-[20px] pb-[20px]">
              ...
              <section className="flex justify-between items-center">
                <p className="">{item?.project_name}</p>
                <div className="flex justify-center items-center">
                  <p className="border-r-[1px] pr-[8px] borderwe">Request ID<span>#REQ-1230</span></p>
                  <div className="ml-[8px] bg-chip-stage1 py-[4px] px-[8px] rounded-[40px]">{item?.stage}</div>
                </div>
              </section>
              <section className="w-[401px] flex justify-between items-center ">
                <div>
                  <p>Expected Delivery by</p>
                  <p>{item?.expected_delivery_by}</p>
                </div>
                <div>
                  <p>Requested by</p>
